...
KImageGray VoronoiGraph::Execute(int obType, int bfType, int threshold, int scale, int borderSize){
    getObstacles(obType, borderSize);
...
Obstacles VoronoiGraph::getObstacles(int type, int borderSize){
    // create obsatcle
    ...
    //바깥 경계에 대한 장애물 생성 알고리즘
    int borderRow=borderSize, borderCol = borderSize;
    for(borderCol = borderSize; borderCol < _igMap.Col() - borderSize; borderCol++){
        obs->push_back(make_pair(borderRow, borderCol));
    }
    borderRow++; borderCol--;
    for(; borderRow < _igMap.Row()-borderSize; borderRow++){
        obs->push_back(make_pair(borderRow, borderCol));
    }
    borderCol--; borderRow--;
    for(; borderCol >= borderSize; borderCol--){
        obs->push_back(make_pair(borderRow, borderCol));
    }
    borderRow--; borderCol++;
    for(; borderRow > borderSize; borderRow--){
        obs->push_back(make_pair(borderRow, borderCol));
    }
