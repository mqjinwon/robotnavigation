...
void MainFrame::on_visibilityMap_clicked()
{
    ...
    for(int p=0; p<lPolygon.size(); p++){
        ...
        if(p == lPolygon.size()-1){
            q_pForm->DrawPolygon(q_Polygon, QColor(255, 255, 255), 1);
        }
        else{
            q_pForm->DrawPolygon(q_Polygon, QColor(255, 255, 255), 1, &Q_colorBrush);
        }
    }
    ...
    for(int i=0; i<plpGraph.size(); i++){
        ...
        for(int n=0; n<pNode->nNeighbors; n++){
            ...
            q_pForm->DrawLine(pNode->nX, pNode->nY, pNeighbor->nX, pNeighbor->nY, QColor(0, 255, 0), 1);
        }
    }
...
void MainFrame::on_visibilityAstar_clicked()
{
    if(mouseClickCnt >= 2){
        ...
        q_pForm->DrawPoints(q_Polygon, QColor(0, 255, 0), 15);
        ...
        q_pForm->DrawPoints(q_Polygon, QColor(255, 0, 0), 15);
        ...
        while(1){
            ...
            q_pForm->DrawLine(result->nX, result->nY, result->opPrvious->nX, result->opPrvious->nY, QColor(255, 255, 0), 3);
            result = result->opPrvious;
        }
...
void MainFrame::on_voronoiDiagram_clicked()
{
    ...
    if(BFimg == 0)
        BFimg = new ImageForm(VRG->Execute(BACKGROUND, EIGHTWAY, thershold, scale, 10), "BRUSHFIRE", this);
...
void MainFrame::on_voronoiAstar_clicked()
{
    if(mouseClickCnt >= 2){
        ...
        if(isRefer){
            ...
            q_Polygon << QPoint(startPos.first, startPos.second);
            BFCimg->DrawPoints(q_Polygon, QColor(0, 255, 0), 7);
            ...
            q_Polygon.clear();
            q_Polygon << QPoint(endPos.first, endPos.second);
            BFCimg->DrawPoints(q_Polygon, QColor(255, 0, 0), 7);
            ...
            while(1){
                ...
                BFCimg->DrawLine(result->nX, result->nY, result->opPrvious->nX, result->opPrvious->nY, QColor(255, 255, 0), 3);
                result = result->opPrvious;
            }
...
void MainFrame::on_odometryMotion_clicked()
{
    ...
    // odometry 변수 설정하는 부분
    odometry->setVarianceParam(ui->alText1->text().toFloat(), ui->alText2->text().toFloat(),
                               ui->alText3->text().toFloat(), ui->alText4->text().toFloat());
...
void MainFrame::on_likelihoodField_clicked()
{
    // make bmp image window
    ImageForm* q_pForm = 0;
    for(int i=0; i < _plpImageForm->Count(); i++){
    ...
    LHF = new LikelihoodField(q_pForm->ImageGray());
    LHF->Execute(BACKGROUND, FOURWAY, thershold, scale, 10);

    QTime startTime = QTime::currentTime();
    ImageForm* LHFimg = 0;
    for(int i=0; i < _plpImageForm->Count(); i++){
        if((*_plpImageForm)[i]->ID() == "LIKELIHOOD"){
            LHFimg = (*_plpImageForm)[i];
            break;
        }
    }

    if(LHFimg == 0){
        LHFimg = new ImageForm(*LHF->likeLihood_field_range_finder_model(), "LIKELIHOOD", this);
        _plpImageForm->Add(LHFimg);
    }
...
void MainFrame::on_PotentialField_clicked(){
    ...
    if(mouseClickCnt >= 2){
        ...
        PTF = new PotentialField(q_pForm->ImageGray());
        ...
        float att       = ui->attParam->toPlainText().toFloat();
        float repul     = ui->repulParam->toPlainText().toFloat();
        float repulNear = ui->repulNearParam->toPlainText().toFloat();

        startTime = QTime::currentTime();
        FVResult = PTF->makePotentialMap(make_pair(startPos.first, startPos.second), make_pair(endPos.first, endPos.second), border, att, repul, repulNear);
        qDebug() << "potential field algorithm time:" << startTime.elapsed();

        RenderArea* PTFImg = new RenderArea();
        PTFImg->setStyleSheet("background:rgb(0,0,0)");
        PTFImg->resize(PTF->width() * sizeUp, q_pForm->height() * sizeUp);
        ...
        PTFImg->setPen(pen);
        PTFImg->setShape(PTFImg->Line);

        //PTFImg->lines.push_back(QLine(0, 0, 150, 150));

        int sizeUpResolution = PTF->getPixelResolution() * sizeUp;

        //여기서 반환된 값으로 이미지를 하나 만들기!
        for(int row= (border + 1) * sizeUp; row < PTFImg->height() - (border + 1) * sizeUp; row+= sizeUpResolution){
            for(int col= (border + 1) * sizeUp; col < PTFImg->width() - (border + 1) * sizeUp; col+= sizeUpResolution){
                //기울기 확인해서 어떤식으로 그릴지 정하는 부분
                double endX = FVResult[(row + sizeUpResolution/2) / sizeUp][(col + sizeUpResolution/2) / sizeUp].first * sizeUp;
                double endY = FVResult[(row + sizeUpResolution/2) / sizeUp][(col + sizeUpResolution/2) / sizeUp].second * sizeUp;

                //qDebug("x : %f, y : %f", endX, endY);

                //좌측 아래에서 화살표를 쏘도록 - red
                if(endX > 0 && endY <= 0){
                    PTFImg->lines[0].push_back(QLine(col + sizeUpResolution/2       , row + sizeUpResolution/2,
                                                     col + sizeUpResolution/2 + endX, row + sizeUpResolution/2 + endY));
                    PTFImg->update();
                }
                //우측 아래에서 화살표를 쏘도록 - green
                else if(endX <= 0 && endY < 0){
                    //qDebug("ex : %f, ey : %f",endX, endY);
                    //qDebug("sx : %d, sy : %d, ex : %f, ey : %f",col, row, col + endX, row + endY);
                    PTFImg->lines[1].push_back(QLine(col + sizeUpResolution/2       , row + sizeUpResolution/2,
                                                     col + sizeUpResolution/2 + endX, row + sizeUpResolution/2 + endY));
                    PTFImg->update();
                }
                //좌측 위에서 화살표를 쏘도록 - cyan
                else if(endX >= 0 && endY >= 0){
                    //qDebug("ex : %f, ey : %f", endX, endY);
                    //qDebug("sx : %d, sy : %d, ex : %f, ey : %f",col, row, col + endX, row + endY);
                    PTFImg->lines[2].push_back(QLine(col + sizeUpResolution/2       , row + sizeUpResolution/2,
                                                     col + sizeUpResolution/2 + endX, row + sizeUpResolution/2 + endY));
                    PTFImg->update();
                }
                //우측 위에서 화살표를 쏘도록 - white
                else if(endX < 0 && endY >= 0){
                    //qDebug("sx : %d, sy : %d, ex : %d, ey : %d",col, row, col + endX, row + endY);
                    PTFImg->lines[3].push_back(QLine(col + sizeUpResolution/2       , row + sizeUpResolution/2,
                                                     col + sizeUpResolution/2 + endX, row + sizeUpResolution/2 + endY));
                }
                else{
                    qDebug("sx : %d, sy : %d, ex : %d, ey : %d",col, row, col + endX, row + endY);
        ...
        PTFImg->lines[4].push_back(QLine(startPos.first * sizeUp, startPos.second * sizeUp, startPos.first * sizeUp, startPos.second * sizeUp));
        PTFImg->lines[4].push_back(QLine(endPos.first * sizeUp, endPos.second * sizeUp, endPos.first * sizeUp, endPos.second * sizeUp));
...
void MainFrame::on_odeSimBT_clicked()
{
    ...
    if(FVResult != nullptr){
        ...
        if(JodeFlag == 1){
            ...
            // map 공유를 위한 변수
            slamMap = q_pForm->ImageGray();

            // odometry 변수 설정하는 부분
            alpha[0] = ui->alText1->text().toFloat();
            alpha[1] = ui->alText2->text().toFloat();
            alpha[2] = ui->alText3->text().toFloat();
            alpha[3] = ui->alText4->text().toFloat();
        ...
        qDebug("please make Potential Field");
    }

}

void MainFrame::on_potentialOdeBT_clicked()
{
    // 포텐셜 필드가 존재시 수행하도록 한다.
    if(FVResult != nullptr){
        odePotentialMap = FVResult;

//        for(int i=0; i< 200; i++){
//            for(int j=0; j< 200; j++){
//                qDebug() << "x: " << j << "y: " << i << "~~~" << odePotentialMap[i][j];
//            }
//        }

        if(Jode->isRunning()){
            //제어 하는 부분을 활성화 시키기
            // - potential field 따에라 움직임 제어를 한다.
            controlFlag = true;
        }
    }
    else{
